...
export function RNode(fetch) {
  ...
  async function fetchJSON(methodUrl, request = undefined) {
    ...
    const opts = request === undefined ? { method: 'GET' }
      : { method: 'POST', body: typeof request === 'string' ? request : JSON.stringify(request) };
  ...
  return freeze({
    ...
    observer: (apiBase) => freeze({
      ...
      findDeploy: deployId => fetchJSON(`${apiBase}/api/deploy/${deployId}`),

      /** @type { (term: string) => Promise<ExploratoryDeployResponse> } */
      exploratoryDeploy: term => fetchJSON(`${apiBase}/api/explore-deploy`, term),
    })
  });
}
